...
from densenet import loadModel
from dataset import loadData
from val import val

def train(opt):
    model, weights, data_dir, epochs, batch_size, learning_rate, resume, device, save_dir = opt.model, opt.weights, \
    opt.data, opt.epochs, opt.batch_size, opt.learning_rate, opt.resume, opt.device, opt.save_dir
    ...
    # Resume
    if resume:
        assert weights.endswith(".pt"), "No weights file find! "
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            loss.backward()
            print("[Epoch: %d/%d], Batch: %d/%d, Loss:%.2f, time:%2f" % (
                epoch, epochs, i + 1, len(train_loader), loss, time.time() - t0))
            optimizer.step()
        scheduler.step()
        ...
        if best_acc == acc:
            ...
            print("TEST ACC: %f, BEST ACC: %f, Save model" % (acc, best_acc))
        else:
...
def parse_opt():
    ...
    parser.add_argument('--model', type=int, default=121, help='select 121, 169, 201, 264')
