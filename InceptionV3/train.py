...
from InceptionV3 import loadModel
...
def train(opt):
    ...
    if resume:
        assert weights.endswith(".pt"), "No Weights File Found"
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            pred = model(images)
            loss = compute_loss(pred[0], labels) + compute_loss(pred[1], labels) * 0.3
...
def parse_opt():
    ...
    parser.add_argument('--model', type=str, default="InceptionV3", help='InceptionV3')
