...
from mobileNetV3 import loadModel
...
def train(opt):
    ...
    schedual = torch.optim.lr_scheduler.StepLR(optimizer, step_size=epochs*0.3, gamma=0.1, last_epoch=-1)
    ...
    # torch.nn.CrossEntropyLoss(predictions, labels)
    # predictions.shape = [N,C]   labels.shape = [N]
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if best_acc == acc:
            ...
            print("TEST ACC: %2.f, Save model" %acc)
        else:
            print("TEST ACC: %2.f" % acc)
        


def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--model', type=str, default="MobileNetV3_s", help='MobileNetV3_s, MobileNetV3_l')
